...
(function () {
	...
	function saveToFile(href, filename) {
	...
	vCard = (function vCard() {
		...
		function getPhoto() {
			return {
				 ...
				 * @param  {string} mediaType Media type of photo (JPEG, PNG, GIF)
				 */
				attachFromUrl: async function (url, mediaType) {
					this.url = url;
				 ...
				 */
				embedFromFile: async function (fileUrl) {
				 ...
				 */
				embedFromString: async function (base64String, mediaType) {
					this.mediaType = mediaType;
		...
		return {
			...
			saveToFile: function (filename) {
				var blob = new Blob([vCardFormatter.getFormattedString(this)], {
					type: "text/vcard"
				});
			...
			/**
			 * Get QRCode from Vcard
			 */
			QRCode: async function (img) {
				return new Promise(async ok => {
					...
					} else if (img && img instanceof HTMLImageElement) {
						image = img;
					}
					var q = new QRCode();
					q.addData(vCardFormatter.getFormattedString(this).replace(/;CHARSET=UTF-8/g,""));
					q.make();

					if (image) {
						image.src = q.createDataURL();
					} else {
						q.download("qrCode");
					}
					ok(q)
...
var qrcode = function() {
   ...
   */
  var qrcode = function(errorCorrectionLevel, typeNumber) {

    var PAD0 = 0xEC;
    var PAD1 = 0x11;

    var _typeNumber = typeNumber || 0;
    var _errorCorrectionLevel = QRErrorCorrectionLevel[errorCorrectionLevel || "H"] || "H";
    var _modules = null;
    var _moduleCount = 0;
    var _dataCache = null;
    var _dataList = [];
    ...
    var createData = function(typeNumber, errorCorrectionLevel, dataList) {
			var buffer, rsBlocks, totalDataCount;
    ...
    _this.getModuleCount = function() {
      return _moduleCount;
		...
		_this.download = async function(filename){
			const blob = await (await fetch(this.createDataURL())).blob(); 
			var href = window.URL.createObjectURL(blob);
			saveToFile(href, filename+".gif");
			window.URL.revokeObjectURL(href);
		}
