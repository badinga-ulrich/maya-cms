...
class Media extends \Maya\AuthController {
    ...
    public function api() {
        ...
        $mediapath = $this->module('maya')->getGroupVar('finder.path', '');

        if (!$mediapath && !$this->module('maya')->isSuperAdmin()) {
            $this->root = rtrim($this->app->path("#uploads:"), '/');
        } else {
            $this->root = MAYA_DIR == MAYA_ENV_ROOT ? rtrim($this->app->path("site:{$mediapath}"), '/') : MAYA_ENV_ROOT;
        }
    ...
    protected function ls() {
        ...
        $isSuperAdmin = $this->module('maya')->isSuperAdmin();
    ...
    protected function upload() {
        ...
        $this->app->trigger('maya.media.upload', [$_uploaded, $_failed]);
    ...
    protected function removefiles() {
        ...
        $this->app->trigger('maya.media.removefiles', [$deletions]);
    ...
    protected function rename() {
        ...
        if ($path && $name && $this->_isFileTypeAllowed($name)) {
            ...
            $this->app->trigger('maya.media.rename', [$source, $target]);
        }
    ...
    protected function _isFileTypeAllowed($file) {
        ...
        $allowed = trim($this->module('maya')->getGroupVar('finder.allowed_uploads', $this->app->retrieve('allowed_uploads', '*')));

        if (strtolower(pathinfo($file, PATHINFO_EXTENSION)) == 'php' && !$this->module('maya')->isSuperAdmin()) {
            return false;
        }
