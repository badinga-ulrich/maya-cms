...
class Auth extends \LimeExtra\Controller {
    ...
    public function check() {
        ...
        if ($auth = $this->param('auth')) {
            ...
            if (!$this->app->helper('csrf')->isValid('login', $this->param('csrf'), true)) {
                $this->app->trigger('maya.authentication.failed', [$auth, 'Csrf validation failed']);
                return ['success' => false, 'error' => 'Csrf validation failed'];
            }

            $user = $this->module('maya')->authenticate($auth);

            if ($user && !$this->module('maya')->hasaccess('maya', 'backend', $user['group'] ?? null)) {
                $user = null;
            }
            ...
            if ($user) {
                ...
                $this->module('maya')->setUser($user);

                unset($user['api_key'], $user['_reset_token']);

            } else {
                $this->app->trigger('maya.authentication.failed', [$auth, 'Authentication failed']);
            }
    ...
    public function login() {
        ...
        return $this->render('maya:views/layouts/login.php', compact('redirectTo'));
    }
    ...
    public function forgotpassword() {
        ...
        return $this->render('maya:views/layouts/forgotpassword.php');
    }
    ...
    public function requestreset() {
        ...
        if ($user = $this->param('user')) {
            ...
            $user = $this->app->storage->findOne('maya/accounts', $query);
            ...
            $message = $this->app->view('maya:emails/recover.php', compact('user','token','target'));
    ...
    public function newpassword() {
        ...
        if ($token = $this->param('token')) {
            ...
            $user = $this->app->storage->findOne('maya/accounts', ['_reset_token' => $token]);
            ...
            return $this->render('maya:views/layouts/newpassword.php', compact('user', 'token'));
        }
