...
class RestApi extends \LimeExtra\Controller {
    ...
    public function authUser() {
        ...
        $user = $this->module('maya')->authenticate($data);
        $generateApiKey = $this->param('generateApiKey', false);

        if (!$user) {
            $this->app->trigger('maya.authentication.failed', [$data['user']]);
            return $this->stop(['error' => 'Authentication failed'], 401);
        }

        $this->app->trigger('maya.authentication.success', [&$user]);

        if ($generateApiKey) {
            $user['api_key'] = 'account-'.\uniqid(\bin2hex(\random_bytes(16)));
            $this->app->storage->save('maya/accounts', $user);
        }
    ...
    public function refreshUserApiKey() {
        ...
        $user = $this->app->storage->findOne('maya/accounts', ['api_key' => $apiKey]);
    ...
    public function saveUser() {
        ...
        if ($user) {
            ...
            $hasAccess = $this->module('maya')->hasaccess('maya', 'accounts');
    ...
    public function listUsers() {
        ...
        if ($user) {
            ...
            $isAdmin = $this->module('maya')->isSuperAdmin($user['group']);
        }
    ...
    public function image() {
        ...
        return $this->module('maya')->thumbnail($options);
    }
    ...
    public function assets() {
        ...
        return $this->module('maya')->listAssets($options);
    }
    ...
    public function asset($id = null) {
        ...
        $assets = $this->module('maya')->listAssets($options);

        return $assets[0] ?? false;
    }

    public function addAssets() {
        return $this->module('maya')->uploadAssets('files', $this->param('meta', []));
    }

    public function updateAssets() {

        if ($asset = $this->param('asset', false)) {
            return $this->module('maya')->updateAssets($asset);
        }
    ...
    public function removeAssets() {
        ...
        if ($assets = $this->param('assets', false)) {
            return $this->module('maya')->removeAssets($assets);
        }
