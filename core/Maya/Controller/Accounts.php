...
class Accounts extends \Maya\AuthController {
    ...
    public function index() {
        ...
        return $this->render('maya:views/accounts/index.php', compact('current', 'groups'));
    }
    ...
    public function account($uid=null) {
        ...
        if (!$this->module('maya')->hasaccess('maya', 'accounts') && $uid != $this->user['_id']) {
            return $this->helper('admin')->denyRequest();
        }

        $account = $this->app->storage->findOne('maya/accounts', ['_id' => $uid]);
        ...
        $this->app->trigger('maya.account.fields', [&$fields, &$account]);

        return $this->render('maya:views/accounts/account.php', compact('account', 'uid', 'languages', 'groups', 'fields'));
    }
    ...
    public function save() {
        ...
        if ($data = $this->param('account', false)) {
            ...
            if (isset($data['group']) && !$this->module('maya')->hasaccess('maya', 'accounts')) {
                unset($data['group']);
            }
            ...
            if (isset($data['user'])) {
                ...
                $_account = $this->app->storage->findOne('maya/accounts', ['user'  => $data['user']]);
            ...
            if (isset($data['email'])) {
                ...
                $_account = $this->app->storage->findOne('maya/accounts', ['email'  => $data['email']]);
            ...
            $this->app->trigger('maya.accounts.save', [&$data, isset($data['_id'])]);
            $this->app->storage->save('maya/accounts', $data);

            $data = $this->app->storage->findOne('maya/accounts', ['_id' => $data['_id']]);

            if (isset($data['password'])) unset($data['password']);
            if (isset($data['_reset_token'])) unset($data['_reset_token']);

            if ($data['_id'] == $this->user['_id']) {
                $this->module('maya')->setUser($data);
            }
    ...
    public function remove() {
        ...
        if ($data = $this->param('account', false)) {
            ...
            if ($data['_id'] != $this->user['_id']) {
                ...
                $this->app->storage->remove('maya/accounts', ['_id' => $data['_id']]);
    ...
    public function find() {
        ...
        $accounts = $this->app->storage->find('maya/accounts', $options)->toArray();
        $count    = (!isset($options['skip']) && !isset($options['limit'])) ? count($accounts) : $this->app->storage->count('maya/accounts', isset($options['filter']) ? $options['filter'] : []);
        ...
        foreach ($accounts as &$account) {
            ...
            $this->app->trigger('maya.accounts.disguise', [&$account]);
        }
    ...
    protected function getLanguages() {
        ...
        foreach ($this->app->helper('fs')->ls('*.php', '#config:maya/i18n') as $file) {
