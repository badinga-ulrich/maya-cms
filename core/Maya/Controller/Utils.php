...
class Utils extends \Maya\AuthController {
    ...
    public function unlockResourceId($resourceId) {
        ...
        if ($meta) {
            ...
            $canUnlock = $this->module('maya')->hasaccess('maya', 'unlockresources');

            if (!$canUnlock) {
                $canUnlock = $meta['sid'] == md5(session_id()) || $this->app->module('maya')->isSuperAdmin();
            }
    ...
    public function startJobRunner() {
        ...
        $this->app->helper('async')->exec("maya()->helper('jobs')->stopRunner();maya()->helper('jobs')->run();");
        sleep(3);
    ...
    public function stopJobRunner() {
        ...
        $this->app->helper('async')->exec("maya()->helper('jobs')->stopRunner();");
        sleep(3);
        return ['running' => $this->app->helper('jobs')->isRunnerActive()];
    }
}
