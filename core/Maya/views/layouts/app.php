...
?><!doctype html>
<html lang="{{ $app('i18n')->locale }}" data-base="@base('/')" data-route="@route('/')" data-version="{{ $app['maya/version'] }}" data-locale="{{ $app('i18n')->locale }}">
<head>
    <meta charset="UTF-8">
    ...
    {{ $app->assets($app('admin')->data->get('assets'), $app['debug'] ? time() : $app['maya/version']) }}

    <script src="@route('/maya.i18n.data')"></script>
...
<body>
    ...
    <script type="riot/tag" src="@base($component)?nc={{ $app['debug'] ? time() : $app['maya/version'] }}"></script>
    @endforeach

    @foreach($app('fs')->ls('*.tag', '#config:tags') as $component)
    <script type="riot/tag" src="{{$app->pathToUrl('#config:tags/'.$component->getBasename())}}?nc={{ $app['debug'] ? time() : $app['maya/version'] }}"></script>
    @endforeach

    @render('maya:views/_partials/logincheck.php')
