...
<head>
    ...
    {{ $app->assets($app['app.assets.base'], $app['maya/version']) }}
    {{ $app->assets(['assets:lib/uikit/js/components/form-password.min.js'], $app['maya/version']) }}
        ...
        <form class="uk-form" method="post" action="@route('/auth/check')" onsubmit="{ submit }">
            ...
            <div id="newpassword-dialog" class="newpassword-dialog uk-panel-box uk-panel-space uk-panel-card uk-nbfc" show="{!$user}">
                ...
                <div class="uk-text-center uk-animation-slide-bottom" if="{ reset }">
                    <a class="uk-button uk-button-outline uk-button-large uk-button-primary uk-width-1-1" href="{{ $app->retrieve('maya.login.url', $app->routeUrl('/auth/login')) }}">@lang('Back to Login')</a>
                </div>
