...
$this->module('maya')->extend([
    ...
    'listAssets' => function($options = []) {
        $this->app->trigger('maya.assets.find', [&$options]);
        $assets = $this->app->storage->find('maya/assets', $options)->toArray();
        $total  = (!isset($options['skip']) && !isset($options['limit']))
                   ? count($assets)
                   : $this->app->storage->count('maya/assets', ($options['filter'] ?? null));

        $this->app->trigger('maya.assets.list', [$assets]);
    ...
    'saveAssets' => function($files, $meta = [], $update = false) {
        ...
        foreach ($files as $idx => &$file) {
            ...
            $asset = [
                ...
                'created' => $created,
                'modified' => $created,
                '_by' => $this->app->module('maya')->getUser('_id')
            ];
            ...
            $this->app->trigger('maya.asset.upload', [&$asset, &$_meta, &$opts, &$file, &$path]);
            ...
            $this->app->trigger('maya.asset.save', [&$asset]);

            $assets[] = $asset;
        }

        if (count($assets)) {
            $this->app->trigger('maya.assets.save', [&$assets]);

            if ($update) {
                foreach ($assets as $asset) {
                    $this->app->storage->save('maya/assets', $asset);
                }
            } else {
                $this->app->storage->insert('maya/assets', $assets);
            }
    ...
    'updateAssetFile' => function($param = 'file', $asset = null) {
        ...
        if (!$file['error'] && $_isAllowed && $_sizeAllowed && move_uploaded_file($file['tmp_name'], $_file)) {
            ...
            $_asset = $this->saveAssets([$_file], [
                ...
                'folder' => isset($asset['folder']) ? $asset["folder"] : ""
            ], true)[0];

            unlink($_file);

            $this->app->trigger('maya.assets.updatefile', [$_asset, $asset]);
    ...
    'removeAssets' => function($assets) {
        ...
        foreach ($assets as &$asset) {
            ...
            if (!isset($asset['path'])) {
                $asset = $this->app->storage->findOne('maya/assets', ['_id' => $asset['_id']]);
            }

            if (!$asset) continue;

            $this->app->storage->remove('maya/assets', ['_id' => $asset['_id']]);
        ...
        $this->app->trigger('maya.assets.remove', [$assets]);
    ...
    'updateAssets' => function($assets) {
        ...
        foreach ($assets as &$asset) {
            ...
            $_asset = $this->app->storage->findOne('maya/assets', ['_id' => $asset['_id']]);

            if (!$_asset) continue;

            $asset['modified'] = time();
            $asset['_by'] = $this->app->module('maya')->getUser('_id');
