...
$this->module('maya')->extend([
    ...
    'authenticate' => function($data) use($app) {
        ...
        $user = $app->storage->findOne('maya/accounts', $filter);
    ...
    'setUser' => function($user, $permanent = true) use($app) {
        ...
        if ($permanent) {
            ...
            $app('session')->write('maya.app.auth', $user);
        }

        $app->trigger('maya.auth.setuser', [&$user, $permanent]);

        $app['maya.auth.user'] = $user;
    },

    'getUser' => function($prop = null, $default = null) use($app) {

        $user = $app->retrieve('maya.auth.user');

        if (is_null($user) && !MAYA_API_REQUEST) {
            $user = $app('session')->read('maya.app.auth', null);
        }
    ...
    'updateUserOption' => function($key, $value) use($app) {
        ...
        if ($user = $this->getUser()) {
            ...
            $app->storage->update('maya/accounts', ['_id' => $user['_id']], ['$set' => ['data' => $data]]);
