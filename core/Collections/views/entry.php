<script type="riot/tag" src="@base('collections:assets/collection-entrypreview.tag')?nc={{ $app['debug'] ? time() : $app['maya/version'] }}"></script>
<script type="riot/tag" src="@base('collections:assets/collection-linked.tag')?nc={{ $app['debug'] ? time() : $app['maya/version'] }}"></script>
<script type="riot/tag" src="@base('collections:assets/entries-table.tag')?nc={{ $app['debug'] ? time() : $app['maya/version'] }}"></script>
...
<div riot-view>
        ...
        <div help="Editeur" class="uk-grid uk-tab-content" show="{viewTab == 'my-entry'}">
            ...
            <div class="uk-grid-margin uk-width-medium-3-4 uk-width-large-4-5">
                <form class="uk-form" if="{ fields.length }" onsubmit="{ submit }">
                    ...
                    <cp-actionbar>
                        <div class="uk-container uk-container-center uk-float-left">
                            <a show="{ entry._id }" href="@route('/collections/entry/'.$collection['name'])" show="{ entry._id }" class="uk-button uk-button-large uk-button-success">@lang('Create an entry')</a>
                            <button show="{ !entry._id }" class="uk-button uk-button-large uk-button-primary">@lang('Save')</button>
                            <a  show="{ !entry._id }" class="uk-button uk-button-link" onclick="history.back()">
                                <span >@lang('Cancel')</span>
                                <span show="{ entry._id }">@lang('Close')</span>
                            </a>
                        </div>
                        <div show="{ entry._id }" class="uk-container uk-container-center uk-float-right">
                ...
                <div class="uk-margin" if="{entry._id}">
                    <div class="uk-margin-small-top">
                        <div class="uk-button-group">
                            ...
                            @if($app->module('maya')->isSuperAdmin())
    ...
    <script type="view/script">
        ...
        this.itemLinked = {{ json_encode($itemLinked) }};
        ...
        this.fields.forEach(function(field) {
            ...
            if($this.itemLinked){
                ...
                field.options.itemLinked = $this.itemLinked;
            }
            /* $this.entry[field.name].itemLinked = $this.itemLinked; */
        ...
        submit(e) {
            ...
            App.request('/collections/save_entry/'+this.collection.name, {entry:this.entry}).then(function(entry) {
                ...
                if (entry) {
                    ...
                    if(!$this.entry._id){
                        window.location.href = App.route('/collections/entry/'+$this.collection.name+'/'+entry._id);
                        return ;
                    }
