...
$this->module('collections')->extend([
    ...
    'save' => function($collection, $data, $options = []) {
        ...
        foreach ($data as &$entry) {
            ...
            // var_dump($collection, $data, $ret, $entry ); exit;
    ...
    '_populate' => function($items, $maxlevel=-1, $level=0, $fieldsFilter = []) {
        ...
        return maya_populate_collection($items, $maxlevel, 0, $fieldsFilter);
    },
...
function maya_populate_collection(&$items, $maxlevel = -1, $level = 0, $fieldsFilter = []) {
    ...
    foreach ($items as $k => &$v) {
        ...
        if (is_array($items[$k])) {
            $items[$k] = maya_populate_collection($items[$k], $maxlevel, ($level + 1), $fieldsFilter);
        }

        if ($level > 0 && isset($v['_id'], $v['link'])) {
            $link = $v['link'];
            $items[$k] = maya('collections')->_resolveLinkedItem($v['link'], (string)$v['_id'], $fieldsFilter);
            $items[$k]['_link'] = $link;
            $items[$k] = maya_populate_collection($items[$k], $maxlevel, ($level + 1), $fieldsFilter);
        }
    }
...
function _check_collection_rule($collection, $rule, $_context = null) {
    ...
    if (isset($collection['rules'][$rule]['enabled']) && $collection['rules'][$rule]['enabled']) {
        ...
        $_rulefile = maya()->path("#storage:collections/rules/{$collection['name']}.{$rule}.php");

        if ($_rulefile) {

            $context->user = maya()->module('maya')->getUser();
            $ret = null;
