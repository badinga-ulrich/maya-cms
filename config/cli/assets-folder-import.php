 ...
 * @version: 0.1.0
 * 
 * @uses ./cp assets-folder-import --dir /path/to/images --ext jpg,png --r
 */
...
if (!$list) {
    $pfid = $app->storage->save('maya/assets_folders', $folder);
...
foreach ($iterator as $file) {
    ...
    if ($file->isDir()) {
        ...
        if (!isset($folders[$pathName])) {
            ...
            if (!$list) {
                $fid = $app->storage->save('maya/assets_folders', $folder);
...
foreach ($files as $k => $file) {
    ...
    $return = $app->module('maya')->addAssets([$file], $meta[$k]);
